...
local backslash, null, newline, carriage, tab, quote, quote2, obracket, cbracket, esc
...
function utils.loadColors (n)
  ...
  esc       = utils.colorize("Bgreen", "\\27", "green")
...
local function escape(byte)
  return utils.colorize("Bgreen", '\\' .. string.byte(byte), "green")
end

function utils.dump(o, depth)
  local t = type(o)
  if t == 'string' then
    return quote .. o:gsub("\27", esc):gsub("\\", backslash):gsub("%z", null):gsub("\n", newline):gsub("\r", carriage):gsub("\t", tab):gsub("[^ %p%w\27]", escape) .. quote2
  end
  if t == 'nil' then
